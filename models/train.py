...
optparser.add_option("-T", "--train", default="data/train.txt", help="Train set location")
optparser.add_option("-d", "--dev", default="data/test.txt", help="Dev set location")
optparser.add_option("-t", "--test", default="data/test.txt", help="Test set location")
...
optparser.add_option(
    "-p",
    "--pre_emb",
    default="embedding/word2vec_vi_words_300dims.txt",
...
optparser.add_option("--name", default="numb_classifier", help="model name")
...
def evaluating(model, datas, best_F):
    ...
    os.system('python %s < %s > %s' % (eval_script, predf, scoref))

    eval_lines = [l.rstrip() for l in open(scoref, 'r', encoding='utf8')]
    for i, line in enumerate(eval_lines):
        print(line)
        if i == 1:
            new_F = float(line.strip().split()[1].replace("%;", ""))
            if new_F > best_F:
                best_F = new_F
                save = True
...
def train():
    ...
    for epoch in range(1, 100):
        pbar = tqdm(np.random.permutation(len(train_data)), desc=f"Epoch {epoch}", position=0)
        for iter, index in enumerate(pbar):
